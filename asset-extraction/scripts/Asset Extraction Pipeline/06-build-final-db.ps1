# Step 6: Build final metadata DB. Phase A: walk output -> existing_paths.csv. Phase B: DuckDB join all -> 5 CSVs. Phase C: PGLite DB from CSVs.
...
param(
    ...
    [string] $OutputFormat = "",   # pglite = build PGLite metadata dir; csv = final_*.csv only; both = DB and CSVs. Default from env or pglite.
    [int] $Workers = 0,   # when > 0, DuckDB uses this many threads (e.g. 1 for single-thread)
    [switch] $SkipExisting,
    [switch] $EnableTrigram,  # create pg_trgm GIN trigram index on tags for fast wildcard substring search
    [switch] $Test
)
...
Ensure-DuckDBTools   # node required for Phase A (walk-output-to-csv.js) and Phase C (create-pglite-db.mjs)
...
$outDbPath = if ([string]::IsNullOrEmpty($OutDb)) { Join-Path $outputDirActual "metadata" } else { $OutDb }
$outDbTmp = $outDbPath + ".tmp"
$imagesDir = Join-Path $outputDirActual "images"
$audioDir = Join-Path $outputDirActual "audio"

$outputFormatActual = if ([string]::IsNullOrWhiteSpace($OutputFormat)) { if ($env:ASSET_EXTRACTION_OUTPUT_FORMAT) { $env:ASSET_EXTRACTION_OUTPUT_FORMAT } else { "pglite" } } else { $OutputFormat }
$enableTrigramActual = if ($EnableTrigram) { $true } elseif ($env:ASSET_EXTRACTION_TRIGRAM -eq "1") { $true } else { $false }
...
# Phase C: PGLite DB from final CSVs
$finalTags = Join-Path $stagingDirActual "final_tags.csv"
$finalAssetTags = Join-Path $stagingDirActual "final_asset_tags.csv"
...
foreach ($f in @($finalTags, $finalAssetTags, $finalAssets, $finalAnimationFrames, $finalCacheLocations)) {
...
if ($outputFormatActual -eq "csv") {
    ...
    Write-Host "Step 6 done. Output: final_tags.csv, final_asset_tags.csv, final_assets.csv, final_animation_frames.csv, final_cache_locations.csv"
    exit 0
}

# pglite or both: run Phase C (PGLite metadata directory). CSVs already in staging from Phase B.
Write-Host "Step 6 Phase C: Creating PGLite metadata database"
$pgliteScript = Join-Path $scriptRoot "helper-scripts\create-pglite-db.mjs"
if (-not (Test-Path $pgliteScript)) { Write-Error "create-pglite-db.mjs not found at: $pgliteScript"; exit 1 }

try {
    if (Test-Path $outDbTmp) { Remove-Item $outDbTmp -Recurse -Force }

    $pgliteArgs = @("--out-dir", $outDbTmp, "--staging-dir", $stagingDirActual)
    if ($enableTrigramActual) { $pgliteArgs += "--enable-trigram" }

    & node $pgliteScript @pgliteArgs
    if ($LASTEXITCODE -ne 0) { Write-Error "Phase C failed (exit $LASTEXITCODE)."; exit $LASTEXITCODE }

    if (Test-Path $outDbPath) { Remove-Item $outDbPath -Recurse -Force }
    Move-Item $outDbTmp $outDbPath -Force
    Write-Host "  Written: $outDbPath"
} finally {
    if (Test-Path $outDbTmp) { Remove-Item $outDbTmp -Recurse -Force -ErrorAction SilentlyContinue }
}
